...
/** Aplica tema guardado sem forçar chave "light" na primeira visita */
...
/** <img src> não envia Bearer — anexos exigem token; usamos blob URL. */
...
/** URLs de anexo exigem Bearer — <img src> não envia token; usamos blob URL como nos avatares. */
var _mentalBranchImageBlobUrls = {};

function revokeMentalBranchImageBlobUrl(nodeId) {
    if (nodeId == null || nodeId === '') return;
    var key = String(nodeId);
    var prev = _mentalBranchImageBlobUrls[key];
    if (prev) {
        try {
            URL.revokeObjectURL(prev);
        } catch (e) {}
        delete _mentalBranchImageBlobUrls[key];
    }
}

function revokeAllMentalBranchImageBlobUrls() {
    Object.keys(_mentalBranchImageBlobUrls).forEach(function(k) {
        try {
            URL.revokeObjectURL(_mentalBranchImageBlobUrls[k]);
        } catch (e) {}
    });
    _mentalBranchImageBlobUrls = {};
}

function mentalAttachmentSrcNeedsAuthFetch(src) {
    if (!src || typeof src !== 'string') return false;
    if (src.indexOf('data:') === 0) return false;
    if (src.indexOf('blob:') === 0) return false;
    try {
        var pathOnly = src.indexOf('://') !== -1 ? new URL(src, typeof window !== 'undefined' ? window.location.origin : undefined).pathname : src;
        return pathOnly.indexOf('/api/attachments/') !== -1;
    } catch (e) {
        return src.indexOf('/api/attachments/') !== -1;
    }
}
...
function applySrcToMentalBranchImage(img, src, nodeId) {
    if (!img) return;
    var nid = nodeId != null && nodeId !== '' ? String(nodeId) : '';
    if (nid) revokeMentalBranchImageBlobUrl(nid);
    img.onload = null;
    img.onerror = null;
    if (!src) {
        img.removeAttribute('src');
    ...
    if (!mentalAttachmentSrcNeedsAuthFetch(src)) {
        img.onerror = function() {
            img.removeAttribute('src');
        };
        img.src = src;
    ...
    var tok = typeof localStorage !== 'undefined' ? localStorage.getItem('token') : null;
    if (!tok) {
        img.removeAttribute('src');
        return;
    }
    var full = src.indexOf('http') === 0 || src.indexOf('//') === 0 ? src : getAttachmentFullUrl(src);
    fetch(full, { headers: { Authorization: 'Bearer ' + tok } })
        .then(function(res) {
        ...
        .then(function(blob) {
            if (!blob || blob.size === 0) throw new Error('empty');
            var ou = URL.createObjectURL(blob);
            if (nid) _mentalBranchImageBlobUrls[nid] = ou;
            img.src = ou;
        })
        .catch(function() {
            var d = window._annotationMentalData;
            var node = nid && d && d.nodes ? d.nodes.find(function(n) { return String(n.id) === nid; }) : null;
            if (node && typeof node.imageData === 'string' && node.imageData.indexOf('data:') === 0) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // auth-guard.js já redireciona para login sem token; aqui assumimos sessão válida ou verificação abaixo.
    ...
    if (token) {
        try {
            ...
            // Sessão inválida ou utilizador já não existe na API (base nova / deploy) — não manter token fantasma.
            const sessionDead =
                em.indexOf('token') !== -1 ||
                em.indexOf('inválido') !== -1 ||
                em.indexOf('invalid') !== -1 ||
                em.indexOf('fornecido') !== -1 ||
                em.indexOf('usuário não encontrado') !== -1 ||
                em.indexOf('usuario nao encontrado') !== -1 ||
                em.indexOf('user not found') !== -1 ||
                em.indexOf('não autorizado') !== -1 ||
                em.indexOf('nao autorizado') !== -1 ||
                em.indexOf('unauthorized') !== -1;
            if (sessionDead) {
...
function renderDashboardOverview() {
    ...
    if (progressEl) {
        ...
        // Sem nenhuma conclusao no dia => estado neutro (cinza), mesmo com tarefas agendadas.
...
async function apiRequest(endpoint, options = {}) {
    try {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            if (/cannot get|cannot post|cannot put|cannot delete/i.test(text)) {
                throw new Error(
                    'O servidor não devolveu dados neste momento. Recarregue a página ou tente outra vez daqui a pouco.'
                );
            }
...
function setupUserProfileModal() {
    ...
    if (btnSave) {
        btnSave.addEventListener('click', async function () {
            ...
            try {
                var body = {
                    name: (document.getElementById('profileEditName') && document.getElementById('profileEditName').value.trim()) || '',
                    sexuality:
                        (document.getElementById('profileEditSexuality') && document.getElementById('profileEditSexuality').value) || '',
                    birthDate:
                        (document.getElementById('profileEditBirth') && document.getElementById('profileEditBirth').value) || ''
                };
                var out = await apiRequest('/profile', { method: 'PUT', body: JSON.stringify(body) });
                if (out.user) {
...
/** Chave por utilizador para última lista do dashboard carregada com sucesso (recuperação se a API falhar). */
function routinesDisplayCacheStorageKey() {
    try {
        var uid = localStorage.getItem('userId') || '';
        return 'ec_routines_display_cache_v1_' + (uid || 'anon');
    } catch (e) {
        return 'ec_routines_display_cache_v1_anon';
    }
}

function readRoutinesDisplayCache() {
    try {
        var raw = localStorage.getItem(routinesDisplayCacheStorageKey());
        if (!raw) return null;
...
function writeRoutinesDisplayCache(routines) {
    if (!Array.isArray(routines) || !routines.length) return;
    try {
        localStorage.setItem(routinesDisplayCacheStorageKey(), JSON.stringify(routines));
    } catch (e) {
        /* quota / modo privado */
    }
}

/** Erros de rede / timeout / HTML inesperado — faz sentido voltar a tentar (ex.: dyno a acordar). */
function isRoutineLoadTransientError(err) {
    var msg = String((err && err.message) || '').toLowerCase();
    return (
        msg.indexOf('failed to fetch') !== -1 ||
        msg.indexOf('networkerror') !== -1 ||
        msg.indexOf('load failed') !== -1 ||
        msg.indexOf('conectar ao servidor') !== -1 ||
        msg.indexOf('não foi possível conectar') !== -1 ||
        msg.indexOf('nao foi possivel conectar') !== -1 ||
        msg.indexOf('recarregue a página') !== -1 ||
        msg.indexOf('recarregue a pagina') !== -1 ||
...
async function loadRoutines(filter = 'all') {
    ...
    function mapRoutinesProgress(routines) {
    ...
    if (token) {
        ...
        try {
            ...
            for (var attempt = 0; attempt < maxAttempts; attempt++) {
                try {
                    routines = await apiRequest('/routines');
                    lastLoadErr = null;
                    break;
                } catch (error) {
                    lastLoadErr = error;
                    if (attempt < maxAttempts - 1 && isRoutineLoadTransientError(error)) {
            ...
            allRoutines = mapRoutinesProgress(allRoutines);
            writeRoutinesDisplayCache(allRoutines);
        } catch (error) {
            lastLoadErr = lastLoadErr || error;
            var cached = readRoutinesDisplayCache();
            var fallbackLocal = [];
            try {
                fallbackLocal = JSON.parse(localStorage.getItem('localRoutines') || '[]');
                if (!Array.isArray(fallbackLocal)) fallbackLocal = [];
            } catch (eLocal) {
                fallbackLocal = [];
            }

            if (cached && cached.length) {
                allRoutines = mapRoutinesProgress(cached);
                try {
                    mergeRoutineProgressFromLocal(allRoutines);
                } catch (eMerge) { /* noop */ }
                alert(
                    'O servidor não respondeu a tempo (por exemplo, o serviço a acordar após inatividade). ' +
                    'A mostrar as últimas rotinas guardadas neste aparelho. Recarregue daqui a um minuto para sincronizar com a nuvem — os dados na conta não foram apagados.'
                );
            } else if (fallbackLocal.length) {
                allRoutines = mapRoutinesProgress(fallbackLocal);
                mergeRoutineProgressFromLocal(allRoutines);
                alert(
                    'Não foi possível carregar as rotinas da nuvem. A mostrar as rotinas só neste aparelho. Tente novamente mais tarde.'
                );
            } else {
                allRoutines = [];
                if (isRoutineLoadTransientError(lastLoadErr)) {
                    alert(
                        'Não foi possível contactar o servidor a tempo. Se usa plano gratuito, o primeiro pedido pode demorar um minuto. ' +
                        'Recarregue a página — as rotinas na base de dados não desaparecem por causa disto.'
                    );
                } else {
                    alert('Não foi possível carregar as rotinas do servidor. Tente novamente.');
                }
            }
        }
    } else {
...
function renderFrequencyChart() {
    ...
    let visStart = 0;
    let visEnd = days.length - 1;
    const visibleSpan = Math.max(1, visEnd - visStart);
    ...
    const y = (v) => {
        const vv = Math.max(0, Math.min(v, maxYVisible));
    ...
    if (!labels.length || labels[labels.length - 1].i !== visEnd) {
        labels.push({ i: visEnd, day: days[visEnd].slice(8, 10) });
    }
    ...
    const finalSeriesIdx = series.reduce((bestIdx, s, idx) => {
        ...
        const bestVal = series[bestIdx] && series[bestIdx].values ? (series[bestIdx].values[visEnd] || 0) : 0;
        const curVal = s && s.values ? (s.values[visEnd] || 0) : 0;
        return curVal > bestVal ? idx : bestIdx;
    }, -1);
...
function renderStatsCharts(routinesToday, completedCountToday) {
    ...
    if (circleEl) {
        ...
        /* Sem conclusoes hoje: neutro (cinza). */
        var circleState = 'neutral';
        var progressStroke = '#9ca3af';
        if (totalToday > 0 && completedCountToday > 0) {
            if (taxaHoje >= 100) {
        ...
        /* Em 0% não desenhar o arco: stroke-linecap round com dash zero vira uma “bolinha” no trilho. */
        const progressArc =
            taxaHoje > 0
                ...
                : '';
        const loadingArc =
            completedCountToday > 0 && taxaHoje < 100
                ? `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="rgba(255,255,255,0.35)" stroke-width="${Math.max(2, stroke - 8)}" stroke-linecap="round" stroke-dasharray="${Math.max(9, dashLen * 0.14)} ${circumference}" stroke-dashoffset="0" class="dashboard-circle-today-loading"/>`
                : '';
        const progressGroup = `<g transform="rotate(-90 ${cx} ${cy})">${progressArc}${loadingArc}</g>`;
        circleEl.innerHTML = `
...
function deleteSelectedTaskAnnotations(routineId, taskId, dateStr, indicesToRemove) {
    ...
    if (!token) {
        try {
            var stored = JSON.parse(localStorage.getItem('localRoutines') || '[]');
            ...
            if (ri !== -1 && stored[ri].tasks) {
                ...
                if (ti !== -1) {
                    ...
                    localStorage.setItem('localRoutines', JSON.stringify(stored));
                }
            }
        } catch (_) {}
    }
...
function ensureHeatmapMobileMediaListener() {
    if (window._ecHeatmapMobileMediaBound) return;
    window._ecHeatmapMobileMediaBound = true;
...
function initAnnotationDigitalizing() {
    ...
    if (!el) return;
    document.querySelectorAll('#annotationEditorDigitalizing .annotation-tool-btn').forEach(function(b) {
        b.classList.toggle('active', b.dataset.cmd === 'highlight');
    });
...
function setMentalBranchImage(branchDiv, fileOrDataUrl) {
    ...
    if (fileOrDataUrl instanceof File && fileOrDataUrl.type && fileOrDataUrl.type.indexOf('image') !== -1) {
        ...
        if (token) {
            ...
            var uploadPromise = readFileAsDataURL(fileOrDataUrl)
                .then(function(tempDataUrl) {
                    if (tempDataUrl && node) {
                        node.imageData = tempDataUrl;
                    }
                    setImageInDom(tempDataUrl || '');
                    return uploadMentalImage(fileOrDataUrl);
                })
                .then(function(ref) {
                    if (ref && node) {
                        node.image = { attachmentId: ref.attachmentId, url: ref.url };
                ...
                .catch(function() {
                    if (node && node.imageData) setImageInDom(node.imageData);
                });
        ...
        readFileAsDataURL(fileOrDataUrl)
            .then(function(tempDataUrl) {
                if (tempDataUrl && node) {
                    node.imageData = tempDataUrl;
                    if (node.image) delete node.image;
                }
                if (tempDataUrl) setImageInDom(tempDataUrl);
            })
            .catch(function() {});
        return;
    }
...
function drawMentalConnections() {
    ...
    function mentalNearlySame(ax, ay, bx, by, eps) {
    ...
    data.edges.forEach(function(edge, idx) {
        ...
        var toH = toR.height / zoom;
        var fromRx = fromC.x - fromW / 2;
        var fromRy = fromC.y - fromH / 2;
        var toRx = toC.x - toW / 2;
        var toRy = toC.y - toH / 2;
        var x1 = fromC.x;
        var y1 = fromC.y;
        var x2 = toC.x;
        var y2 = toC.y;
        if (fromW > 2 && fromH > 2) {
            var fe = segmentRectIntersection(fromC.x, fromC.y, toC.x, toC.y, fromRx, fromRy, fromW, fromH);
            if (!mentalNearlySame(fe.x, fe.y, toC.x, toC.y, 0.5)) {
                x1 = fe.x;
                y1 = fe.y;
            } else {
                var ddfx = toC.x - fromC.x, ddfy = toC.y - fromC.y, llf = Math.sqrt(ddfx * ddfx + ddfy * ddfy) || 1;
                var pullF = Math.min(20 / zoom, llf * 0.25);
                x1 = fromC.x + (ddfx / llf) * pullF;
                y1 = fromC.y + (ddfy / llf) * pullF;
            }
        }
        if (toW > 2 && toH > 2) {
            var te = segmentRectIntersection(fromC.x, fromC.y, toC.x, toC.y, toRx, toRy, toW, toH);
            if (!mentalNearlySame(te.x, te.y, toC.x, toC.y, 0.5)) {
                x2 = te.x;
                y2 = te.y;
            } else {
                var ddx = toC.x - fromC.x, ddy = toC.y - fromC.y, len = Math.sqrt(ddx * ddx + ddy * ddy) || 1;
                var pull = Math.min(24 / zoom, len * 0.35);
                x2 = toC.x - (ddx / len) * pull;
                y2 = toC.y - (ddy / len) * pull;
            }
        }
        var midX = (x1 + x2) / 2;
        var midY = (y1 + y2) / 2;
...
async function saveTaskAnnotation(routineId, taskId, annotation, saveName) {
    ...
    if (isSynthetic) {
        try {
            ...
            var tok = localStorage.getItem('token');
            if (tok) {
                showToast('Não foi possível criar a tarefa no servidor. Tente novamente.');
            ...
            try {
                const stored = JSON.parse(localStorage.getItem('localRoutines') || '[]');
    ...
    try {
        ...
        if (res.status === 401) {
            localStorage.removeItem('token');
            localStorage.removeItem('userName');
            showToast('Sessão expirada. Faça login novamente para continuar.', 6000);
            if (typeof renderAgenda === 'function') renderAgenda();
        ...
        var tok = localStorage.getItem('token');
        if (tok) {
            showToast('Não foi possível salvar no servidor. Tente novamente.', 6000);
            return { savedToServer: false, was401: false };
        }
        try {
            var stored = JSON.parse(localStorage.getItem('localRoutines') || '[]');
...
function setupAnnotationModal() {
    ...
    document.querySelectorAll('#annotationEditorDigitalizing .annotation-tool-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const el = document.getElementById('annotationDigitalContent');
            if (!el) return;
            const bar = btn.closest('.annotation-digital-toolbar');
            if (bar) bar.querySelectorAll('.annotation-tool-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');

...
async function getAttachmentMeta(attachmentId) {
    const { rows } = await pool.query(
        'SELECT user_id, disk_filename, mime_type FROM attachments WHERE id = $1',
        [attachmentId]
    );
    if (!rows[0]) return null;
    return {
        userId: rows[0].user_id,
        filename: rows[0].disk_filename,
...
async function getAdminSummary() {
    const [u, r, u7, r7, a] = await Promise.all([
        pool.query('SELECT COUNT(*)::int AS c FROM users'),
        pool.query('SELECT COUNT(*)::int AS c FROM routines'),
        pool.query("SELECT COUNT(*)::int AS c FROM users WHERE created_at >= now() - interval '7 days'"),
        pool.query("SELECT COUNT(*)::int AS c FROM routines WHERE updated_at >= now() - interval '7 days'"),
        pool.query('SELECT COUNT(*)::int AS c FROM attachments')
    ]);
    return {
        usersCount: u.rows[0].c,
        routinesCount: r.rows[0].c,
        usersCreatedLast7Days: u7.rows[0].c,
        routinesUpdatedLast7Days: r7.rows[0].c,
        attachmentsCount: a.rows[0].c
    };
}
